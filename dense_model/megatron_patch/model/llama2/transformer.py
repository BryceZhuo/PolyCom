...
class PolyNorm(nn.Module):
    ...
    def forward(self,x):
        return torch.utils.checkpoint.checkpoint(polynorm, x, self.weight, self.bias, use_reentrant=False)
...
class PolyReLU(nn.Module):
    ...
    def forward(self,x):
        x = F.relu(x)
        return torch.utils.checkpoint.checkpoint(poly, x, self.weight, self.bias, use_reentrant=False)
...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, config, is_expert=False):
        ...
        if args.openai_gelu:
            ...
            self.activation_func = PolyNorm()
        elif args.polyrelu:
            self.activation_func = PolyReLU()
        else:
