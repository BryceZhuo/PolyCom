...
class Activation(nn.Module):
    ...
    def build(cls, config: ModelConfig) -> Activation:
        ...
        elif config.activation_type == ActivationType.polynorm:
            return PolyNorm(config)
        elif config.activation_type == ActivationType.polyrelu:
            return PolyReLU(config)
        else:
...
class PolyNorm(Activation):
    def __init__(self,config: ModelConfig):
        super().__init__(config)
        self.weight = nn.Parameter(torch.ones(3, device=config.init_device)/3)
        self.bias = nn.Parameter(torch.zeros(1, device=config.init_device))
    
    def forward(self,x):
        raw_shape = x.shape
        x = x.reshape(-1, int(self.config.mlp_hidden_size/raw_shape[-2]), raw_shape[-2], raw_shape[-2])
        x = x.permute(0,2,1,3).contiguous()
        x = x.reshape(x.shape[0], x.shape[1], -1)
        result = torch.utils.checkpoint.checkpoint(polynorm, x, self.weight, self.bias, use_reentrant=False)
        result = result.reshape(x.shape[0], x.shape[1], int(self.config.mlp_hidden_size/raw_shape[-2]), raw_shape[-2])
        result = result.permute(0,2,1,3).contiguous()
        result = result.reshape(raw_shape)
        return result
    ...
    def reset_parameters(self):
        torch.nn.init.constant_(self.weight,1/3)
...
class PolyReLU(Activation):
    ...
    def forward(self,x):
        ...
        x = F.relu(x)
        result = torch.utils.checkpoint.checkpoint(poly, x, self.weight, self.bias, use_reentrant=False)
        return result
...
class OLMoEBlock(OLMoBlock):
    ...
    def reset_parameters(self):
        self.act.reset_parameters()
        if self.moe_args.mlp_type == 'mlp':
            self.ffn.experts.mlp.args.activation_fn = self.act
        if self.k_norm is not None:
            self.k_norm.reset_parameters()
        if self.q_norm is not None:
            self.q_norm.reset_parameters()
